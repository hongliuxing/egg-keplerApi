...
export default class ActionTokenService extends Service {
    ...
    */
    ...
    public async decode(token: string): Promise<any> {
        return this.ctx.jwt.decode(token, { complete: true });
    }
    ...
    public async blackToken(token: string) {
        ...
        // 储存到缓存黑名单中去
        return this.ctx.service.cache.set(this.cachePrefix + key, 'black_success', refreshTtlTime);
    }
    ...
    public async verifyBlack(token: string): Promise<boolean> {
        ...
        // 返回获取该缓存
        return this.ctx.service.cache.has(this.cachePrefix + key);
    }
